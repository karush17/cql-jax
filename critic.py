...
def get_values(critic: Model, state: jnp.ndarray, actions: jnp.ndarray, critic_params: Params):
...
def update_cql(key: PRNGKey, actor: Model, critic: Model, target_critic: Model,
           temp: Model, lag: jnp.ndarray, lagrange_thresh: float, min_q_weight: float,
           batch: Batch, discount: float, soft_critic: bool) -> Tuple[Model, InfoDict]:
    dist = get_actions(actor, batch.observations)
    actions = dist.sample(seed=key)
    log_probs = jnp.reshape(dist.log_prob(actions), (batch.observations.shape[0], NUM_REPEAT, 1))

    next_dist = get_actions(actor, batch.next_observations)
    next_actions = next_dist.sample(seed=key)
    next_log_probs = jnp.reshape(dist.log_prob(next_actions), (batch.observations.shape[0], NUM_REPEAT, 1))

    rand_actions = uniform(key, (actions.shape[0],actions.shape[1]), minval=-1, maxval=1)
    ...
    def critic_loss_fn(critic_params: Params) -> Tuple[jnp.ndarray, InfoDict]:
        ...
        rand_q1, rand_q2 = get_values(critic, batch.observations, rand_actions, critic_params)
        ...
        q1_pred, q2_pred = critic.apply_fn({'params': critic_params}, batch.observations,
                                 batch.actions)
