...
def _update_jit(
    ...
    target_entropy: float, lagrange_thresh: float, min_q_weight: float, update_target: bool
) -> Tuple[PRNGKey, Model, Model, Model, Model, InfoDict]:

    rng, key = jax.random.split(rng)
    new_critic, critic_info = update_cql_critic(key, actor, critic, target_critic,
                                                temp, lag, lagrange_thresh, min_q_weight,
...
class CQLLearner(object):
    def __init__(self,
        ...
        """
        An implementation of the version of CQL described in https://arxiv.org/pdf/2006.04779.pdf
        """
    ...
    def update(self, batch: Batch) -> InfoDict:
        ...
        if self.step > self.start_step:
            new_rng, new_actor, new_critic, new_target_critic, new_temp, info = _update_jit(
                ...
                self.lag_val, batch, self.discount, self.tau, self.target_entropy,
                self.lagrange_thresh, self.min_q_weight, self.step % self.target_update_period == 0)
        else:
            new_rng, new_actor, new_critic, new_target_critic, new_temp, info = _bc_jit(
                self.rng, self.actor, self.critic, self.target_critic, self.temp,
                self.lag_val, batch, self.discount, self.tau, self.target_entropy,
                self.lagrange_thresh, self.min_q_weight, self.step % self.target_update_period == 0)
        ...
        if self.with_lagrange:
            new_lag, lag_info = temperature.update_lag(self.lag, info['gap1'], info['gap2'],
